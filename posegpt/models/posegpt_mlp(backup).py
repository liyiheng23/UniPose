...
class PoseGPTModelFullMask(PoseGPTModel):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if torch.any(input_ids == -1): # 必须是pose begin idx + [-1, ] * 1, replace -1 to pose embed
                assert input_ids.shape[-1] == 2
                inputs_embeds = torch.concat([self.embed_tokens(input_ids[:, :1]), 
                                              self.embed_tokens.weight[None, -1:, :].repeat(input_ids.shape[0], 1, 1)], dim=1) # type: ignore
            else:
...
class PoseGPTMLP(PoseGPT):
    def __init__(self, config, **kwargs):
        ...
        self.smpl_decoder.apply(self.my_init_weights)
    
    # 定义初始化函数
    def my_init_weights(self, m):
        if isinstance(m, nn.Linear):
            # 对线性层的权重使用 Xavier 初始化
        ...
        **kwargs
        ...
        if self.training:
            ...
            for i, label in enumerate(labels):
                ...
                # pose loss
                pose_embd = hidden_states[i][pose_begin_idx + 1:pose_end_idx]  # 1, c
                pose_embd = self.smpl_decoder(pose_embd)
                pose_embd = pose_embd.view(-1, 22, 6)
                pose_rotmat = rotation_6d_to_matrix(pose_embd.to(torch.float16)).view(-1, 22, 3, 3)
                loss_hmr = (pose_rotmat - body_poseA_rotmat[i:i + 1]).abs().mean()
                ...
                all_loss.append([loss_prefix_texts + loss_posefix_texts, loss_hmr])

            _hmr_loss = 0
            _loss = 0
            for _loss_i, _loss_hmr in all_loss:
                _hmr_loss = _hmr_loss + _loss_hmr
                _loss = _loss + _loss_i
            
            _hmr_loss = _hmr_loss / (len(all_loss) + 1e-8)
            print(_hmr_loss)
            _loss = _loss / ((labels != -100).sum() - labels.shape[0] + 1e-8)
            loss = _loss + _hmr_loss * 0.1            
        if not return_dict:
        ...
        if self.training:
            for i, task in enumerate(tasks):
                ...
                if POSE_TOKEN in task_output or POSEB_TOKEN in task_output:
                    ...
                    attention_mask[i][pose_begin_idx + 1:pose_begin_idx + 1 + 1] = 2 # set mask as 2 when need full mask
                    new_input_embeds[i][pose_begin_idx + 1:pose_begin_idx + 1 + 1] = self.get_model().embed_tokens.weight[-1:, :]
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if next_pose_prediction:
                hidden_states = outputs['hidden_states'][-1][:, 1] # B, C
                rot_6d = self.smpl_decoder(hidden_states)
                aa = matrix_to_axis_angle(rotation_6d_to_matrix(rot_6d.view(-1, 22, 6).cpu()))
                vis_mesh(aa[0].flatten())
            ...
            if next_pose_prediction:
                ...
                next_tokens = torch.concat([next_tokens, -torch.ones_like(next_tokens).repeat(1, 1)], dim=1)
            else:
    ...
    def _update_model_kwargs_for_generation(
        ...
        if "attention_mask" in model_kwargs:
            ...
            if next_pose_prediction:
                ...
                attention_mask = torch.cat([attention_mask, 2*attention_mask.new_ones((attention_mask.shape[0], 1))], dim=-1)
