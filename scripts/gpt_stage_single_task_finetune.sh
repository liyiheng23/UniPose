...
STAGE=image2text-full-mask-finetune
EPOCH=6
TRAIN_LLM=True
TRAIN_MM_PROJECTOR=True
LOAD_FROM=./checkpoints/stage-image2text-full-mask-pretrain-epoch-2/checkpoint-3126
deepspeed \
    --include=localhost:0,1,2,3 \
    --master_port=29500 \
    scripts/gpt_train_full_mask.py \
    --deepspeed ./scripts/zero2.json \
    --config configs/posegpt_finetune_single_task.py \
    --lora_enable True \
