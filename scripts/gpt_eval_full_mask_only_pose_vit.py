...
def load_pretrained_model(config, model_path, model_base, device_map='auto', torch_dtype=None, **kwargs):
    ...
    model.model.mm_projector[0].weight = torch.nn.Parameter(torch.empty(4096, 1280, device=model.device, dtype=model.dtype))
...
def eval_model(args):
    ...
    # build dataset
    data_module = build_data_module(eval_dataset_config=config.data.eval, image_processor=image_processor)
