...
from posegpt.models.posegpt import PoseGPTCLIP
from posegpt.datasets.posegpt import build_data_module
from llava.train.train import (
    ModelArguments, TrainingArguments, LLaVATrainer, 
...
class CustomTrainingArguments(TrainingArguments):
    ...
    # llm + word embed + lm_head
    train_llm: Optional[bool] = field(default=False)
    # mm_projector
    train_mm_projector: Optional[bool] = field(default=False)
    # hmr_vit
    train_hmr_vit: Optional[bool] = field(default=False)
    train_vision: Optional[bool] = field(default=False)

def find_all_linear_names(model, train_hmr_backbone=False, train_llm=False):
    cls = torch.nn.Linear
    lora_module_names = set()
    # can't train these modules using lora
    multimodal_keywords = ['mm_projector', 'vision_tower', 'vision_resampler', 'lm_head', 'pose_vqvae']
    for name, module in model.named_modules():
        if any(mm_keyword in name for mm_keyword in multimodal_keywords):
            continue
        if isinstance(module, cls):
            # add hmr backbone
            if train_hmr_backbone and 'hmr_vit_backbone' in name:
                lora_module_names.add(name)
            # add llm
            if train_llm and 'hmr_vit_backbone' not in name:
                lora_module_names.add(name)
...
def load_tokenizer(model_args, training_args, codebook_size):
    ...
    tokenizer.add_tokens([f'<pose_id_{codebook_size}>', f'<pose_id_{codebook_size + 1}>'])
    # use img start and end token
    tokenizer.add_tokens([DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN], special_tokens=True)
    # ! tobe verified: modify output pose mask to learnable pose embed
    # num_tokens = 80
    # tokenizer.add_tokens([f'<pose_query_{i}>' for i in range(num_tokens)])
    # ! tobe verified: modify output pose mask to learnable pose embed
    return tokenizer

def load_model(config, model_args, training_args, data_args, tokenizer, codebook_size, attn_implementation):
    model = PoseGPTCLIP.from_pretrained(
        model_args.model_name_or_path,
        cache_dir=training_args.cache_dir,
    ...
    model.config.use_cache = False
    # model.config.mm_hidden_size = 1024 + 1280
...
def train(attn_implementation=None):
    ...
    train_vision = training_args.train_vision
    ...
    if training_args.lora_enable:
        ...
        if train_llm and not train_mm_projector and not load_from and not train_hmr_vit:
            ...
            # * stage 3, finetune mm_projecror and llm
                ...
                raise NotImplementedError # 这个loss会变0
                from peft import LoraConfig, get_peft_model
                lora_config = LoraConfig(
                    r=training_args.lora_r,
                    lora_alpha=training_args.lora_alpha,
                    target_modules=find_all_linear_names(model, train_hmr_backbone=True, train_llm=True),
                    lora_dropout=training_args.lora_dropout,
                    bias=training_args.lora_bias,
                    task_type="CAUSAL_LM")
                # load llm lora weight
                model = PeftModel.from_pretrained(model, training_args.load_from, is_trainable=True)
                model = model.merge_and_unload()
                # add hmr and llm to lora
                model = get_peft_model(model, lora_config)
            else:
    ...
    # * unfreeze vision module
    # ! for ablation
    # for n, p in model.named_parameters():
    #     if any([x in n for x in ["vision"]]):
    #         p.requires_grad_(True)
